...
// import BikeLayout from '../../AvailableVehicles/Layouts/BikeLayout';
// import { 
//   manualBookSeat, 
...
//           {/* Bus layout */}
//           <div className="w-600 mx-auto  border rounded-lg shadow-inner p-4 bg-gray-50">
//             <BikeLayout 
...
const ManualSeatBookingBike = () => {
  ...
  const validateForm = () => {
    if (!selectedSeats.length) {
      setBookingStatus("Please select a seat");
      setIsError(true);
      return false;
    }
    ...
    if (!contactRegex.test(passengerContact)) {
      setBookingStatus("Contact number must be exactly 10 digits");
      setIsError(true);
      return false;
    }
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      // We'll reuse this same object for each seat booking
      const baseBookingData = {
        passengerName: passengerName.trim(), // Remove any leading/trailing spaces
      ...
      if (successCount === selectedSeats.length) {
        // All seats were booked successfully
        setBookingStatus(`Successfully booked seat`);
        setIsError(false);
  ...
  // Handle input validation for passenger name (prevent spaces at beginning)
  const handleNameChange = (e) => {
    const value = e.target.value;
    // Only update if the first character is not a space
    if (value === '' || value[0] !== ' ') {
      setPassengerName(value);
    }
  };

  // Handle input validation for contact number (only digits, max 10)
  const handleContactChange = (e) => {
    ...
    if (digitsOnly.length <= 10) {
      setPassengerContact(digitsOnly);
    }
  };
  ...
  return (
    <div className="flex flex-col md:flex-row md:space-x-4 px-4 md:px-6 py-6 max-w-6xl mx-auto">
      {/* Left Column - Bike Layout */}
      <div className="flex-1 mb-6 md:mb-0">
        <div className="bg-white rounded-lg shadow-md p-4 h-full flex flex-col">
          <h2 className="text-xl font-semibold mb-4 text-center text-blue-700">Select Seat</h2>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <button
                  onClick={handleBooking}
                  disabled={isLoading || selectedSeats.length === 0}
                  className={`w-full py-3 px-4 rounded-md transition font-medium ${
                    isLoading || selectedSeats.length === 0
                      ? "bg-blue-400 cursor-not-allowed" 
                      : "bg-blue-600 hover:bg-blue-700"
                  } text-white`}
