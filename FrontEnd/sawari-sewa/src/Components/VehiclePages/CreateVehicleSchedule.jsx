...
const CreateVehicleSchedule = () => {
  ...
  useEffect(() => {
    if (drivers.length > 0) {
      const filtered = drivers.filter(driver => {
        ...
        const vehicleInfo = `${driver.vehicleNumber}-${driver.vehicleType}`.toLowerCase();
  ...
  const loadApprovedDrivers = async () => {
    try {
      ...
      // Fetch all approved drivers
      const driverResponse = await axios.get(
        `${API_URL}/Driver/all-drivers-forSchedule`, 
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
            "Content-Type": "application/json",
  ...
  const handleSelectDriver = (driver) => {
    ...
    setSearchQuery(`${driver.firstName} ${driver.lastName} - ${driver.vehicleNumber}`);
            ...
            {drivers.length === 0 ? (
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                {isDropdownOpen && filteredDrivers.length > 0 && (
                  <div 
                    className="absolute z-10 w-full mt-1 max-h-60 overflow-y-auto bg-white border border-gray-300 rounded shadow-lg"
                  >
                    {filteredDrivers.map(driver => (
                      ...
                      >
                        ...
                        <p className="text-sm text-gray-600">
                          ID: {driver.id} | Vehicle: {driver.vehicleNumber}-{driver.vehicleType}
                        </p>
                      </div>
                    ))}
                  </div>
                )}
                {isDropdownOpen && searchQuery && filteredDrivers.length === 0 && (
