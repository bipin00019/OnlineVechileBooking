...
namespace SawariSewa.Areas.Admin.Controllers
{
    ...
    public class OtpController : ControllerBase
    {
        ...
        private readonly ILogger<OtpController> _logger;
        private readonly UserManager<ApplicationUser> _userManager;
        private static string _globalVerificationCode; // Global variable for verification code
        private static string _globalEmail; // Global variable for email
        private static DateTime _codeGeneratedTime; // Global variable for code timestamp
        private readonly TimeSpan _codeExpiryTime = TimeSpan.FromMinutes(5);

        public OtpController(IEmailService emailService, ILogger<OtpController> logger, UserManager<ApplicationUser> userManager)
        {
            _emailService = emailService;
            _logger = logger;
            _userManager = userManager; // Inject UserManager for checking user existence
        ...
        // Endpoint to send a verification code for registration
        [AllowAnonymous]
        [HttpPost("send-code-for-register")]
        public async Task<IActionResult> SendVerificationCodeRegister([FromBody] string email)
        {
        ...
        // Endpoint to send a verification code (useful for other scenarios)
        [AllowAnonymous]
        [HttpPost("send-code")]
        public async Task<IActionResult> SendVerificationCode([FromBody] string email)
        {
    ...
    // Model to capture the verification code request
    public class VerifyCodeRequest
    {
        public string Email { get; set; }
        public string Code { get; set; }
